...
const allowRecognizerOverride =
  import.meta.env.DEV || import.meta.env.MODE === 'test';

export function hasRecognizerOverride(): boolean {
  return allowRecognizerOverride
    && typeof window !== 'undefined'
    && Boolean(window.__captureRecognizers);
}
...
export function getRecognizers(): CaptureRecognizers {
  const override = hasRecognizerOverride()
    ? window.__captureRecognizers
    : undefined;
  return {
