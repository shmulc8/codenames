...
test.describe('mobile pan and zoom board', () => {
  test('renders all 25 tiles on a fit-to-screen canvas with canonical roles', async ({ page }) => {
    ...
    await expect(canvas.locator('[data-board-transform]')).toHaveAttribute(
      'data-at-fit',
      'true',
    );
  });
  ...
  test('a large pointer movement pans without opening the action sheet and fit resets it', async ({ page }) => {
    await mountMobileBoard(page);
    await setFixtureBoard(page);
  ...
  test('pinches between fit and one-card scale and double-tap toggles point zoom', async ({ page }) => {
    ...
    await dispatchPointer(page, 'tile-0', 'pointerdown', { clientX: 120, clientY: 260, pointerId: 1 });
    await dispatchPointer(page, 'tile-1', 'pointerdown', { clientX: 240, clientY: 260, pointerId: 2 });
    await dispatchPointer(page, 'tile-0', 'pointermove', { clientX: 70, clientY: 260, pointerId: 1 });
    await dispatchPointer(page, 'tile-1', 'pointermove', { clientX: 300, clientY: 260, pointerId: 2 });
    ...
    await dispatchPointer(page, 'tile-1', 'pointerup', { clientX: 300, clientY: 260, pointerId: 2 });

    await page.getByTestId('btn-fit-board').click();
    await expect(layer).toHaveAttribute('style', fitTransform ?? '');
    for (let tap = 0; tap < 2; tap += 1) {
      await dispatchPointer(page, 'tile-2', 'pointerdown', { clientX: 210, clientY: 300, pointerId: 1 });
      await dispatchPointer(page, 'tile-2', 'pointerup', { clientX: 210, clientY: 300, pointerId: 1 });
    }
    await expect(layer).toHaveAttribute('data-at-fit', 'false');
    for (let tap = 0; tap < 2; tap += 1) {
      await dispatchPointer(page, 'tile-2', 'pointerdown', { clientX: 210, clientY: 300, pointerId: 1 });
  ...
  test('snaps rubber-band pan immediately when reduced motion is preferred', async ({ page }) => {
    ...
    await dispatchPointer(page, 'tile-0', 'pointerdown', { clientX: 90, clientY: 260, pointerId: 1 });
    await dispatchPointer(page, 'tile-0', 'pointermove', { clientX: 150, clientY: 300, pointerId: 1 });
    await dispatchPointer(page, 'tile-0', 'pointerup', { clientX: 150, clientY: 300, pointerId: 1 });
  ...
  test('marks and restores a tile, defaulting chosenBy to its own role', async ({ page }) => {
    ...
    await dispatchPointer(page, 'tile-9', 'pointerdown', { clientX: 130, clientY: 280, pointerId: 1 });
    await dispatchPointer(page, 'tile-9', 'pointerup', { clientX: 130, clientY: 280, pointerId: 1 });
    ...
    await expect.poll(() => page.evaluate(() => window.__store?.getState().tiles[9])).toMatchObject({
      lifecycle: 'chosen',
      chosenBy: 'blue',
    });

    await dispatchPointer(page, 'tile-9', 'pointerdown', { clientX: 80, clientY: 250, pointerId: 1 });
  ...
  test('routes clue selection through the store and surfaces cluster errors as a toast', async ({ page }) => {
    await mountMobileBoard(page);
    await setFixtureBoard(page);

    await dispatchPointer(page, 'tile-0', 'pointerdown', { clientX: 80, clientY: 250, pointerId: 1 });
    ...
    await expect.poll(() => page.evaluate(() => window.__store?.getState().selected)).toEqual([
      fixtureBoard.words[0],
    ]);
  });

  test('shows the store-driven game-over banner when the assassin is revealed', async ({ page }) => {
    await mountMobileBoard(page);
    await setFixtureBoard(page);

    await dispatchPointer(page, 'tile-24', 'pointerdown', { clientX: 80, clientY: 250, pointerId: 1 });
    await dispatchPointer(page, 'tile-24', 'pointerup', { clientX: 80, clientY: 250, pointerId: 1 });
    await page.getByTestId('btn-mark-chosen').click();
