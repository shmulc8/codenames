# Codenames Copilot — Design Handoff

Hebrew Codenames (שם-קוד) **Spymaster copilot**. This package hands the redesign to Claude Code / a developer. The living source of truth for pixels is the design file `Codenames Copilot Redesign.dc.html`; this doc + `tokens.css` + `screens/*.png` + `simple-html/*` are the extract.

> **Read `SKILL.md`, `ui-changes.md` and `missing-ui-gaps.md` (in `uploads/`) alongside this.** They carry the game rules and the full change rationale. This doc is the _design decisions_; those are the _requirements_.
...
**Implication for devs:** no `myTeam` state anywhere. The clue endpoint takes _a set of selected card ids that share a color_ + risk level. Board state is one model (below), not "my board vs their board".
...
`tokens.css` in this folder mirrors those tokens **plus** the app-only `--cn-*` tokens (role colors, cream card, map, dark chrome). Take every value from a token; don't invent hues outside this set.
...
**Revealed/chosen card**: tinted agent tile (role gradient), struck word, ✓ (or ✗ for assassin) top-right, role shape top-left. The tint _is_ the "who claimed it" cue — but keep the shape/✓ so it survives colorblind/grayscale.
...
`mobile-3e-map.png` + the map panel in `desktop-3a-main.png`; reference art in `uploads/semantic-map-reference.svg`.
...
handoff/
├── DESIGN.md               ← this file
├── tokens.css              ← Nocturne + --cn-* app tokens
├── simple-html/
...
Pixel source of truth: `Codenames Copilot Redesign.dc.html` (open in a browser). Requirements & rationale: `uploads/ui-changes.md`, `uploads/missing-ui-gaps.md`, `uploads/SKILL.md`.
