...
0. **Team-agnostic model (read CONTRACTS §1–§3 + DESIGN.md §1 first).** The app is an analyzer of the board in front of you — it **never asks "which team are you on."** Every tile carries its REAL key-card color (`red|blue|neutral|assassin`); there is NO `myColor`/"mine/opponent". A clue is always requested _for one team color_ (`store.target`), selecting a same-color cluster. The engine's `my|opp` wire vocabulary lives ONLY inside `src/api/client.ts` (CONTRACTS §1 mapping).
