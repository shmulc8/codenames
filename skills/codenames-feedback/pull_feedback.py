...
Run from the repo root: python skills/codenames-feedback/pull_feedback.py
...
def main() -> int:
    ...
    print(f"{DATASET}: {len(rows)} rows  |  👍 {up}  👎 {down}"
    ...
    for r in sorted(rows, key=lambda r: r.get("ts", 0)):
        ...
        print(f"{t} {v} {r.get('clue','')!r}·{r.get('count')} → {','.join(r.get('intended') or [])}  💬 {c}{flag}")
    if not any_c:
        print("(none)")
    return 0
