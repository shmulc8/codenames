...
def main() -> int:
    ...
    for clue, w0, want_illegal in [
        ...
        check(
            f"{clue} vs {w0} → illegal={got}",
            st == 200 and got == want_illegal,
            f"(want {want_illegal})",
        )
    ...
    for risk in ("cautious", "balanced", "bold"):
        st, r = post(
            base, "/api/coach/spymaster", {"words": b.words, "roles": b.role, "risk": risk}
        )
        good = st == 200 and isinstance(r, dict) and r.get("clue")
        check(
            f"{risk} clue",
            good,
            f"→ {r.get('clue')}·{r.get('count')}" if isinstance(r, dict) else str(r),
        )
