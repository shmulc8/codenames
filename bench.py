...
SPY_ENC = "fasttext"          # the spymaster geometry (Hebrew word-similarity champion: 100% on a triplet probe)
...
def judge(llm, board, clue):
    user = (f"מילות הצוות: {', '.join(board.my)}\n"
...
def spymaster(cfg, board, A):
    ...
    if cfg["engine"] == "geometry":
        return probe.encoder_spymaster(A["enc"], board, words, emb, vocab_lemmas=lems,
                                       lam_f=lam_f, vocab_freq=fz, m=cfg["m"])
    cands = probe.encoder_clue_candidates(A["enc"], board, words, emb, vocab_lemmas=lems,
                                          n=10, lam_f=lam_f, vocab_freq=fz, m=cfg["m"])
...
def evaluate(clue, board, A, do_judge):
    ...
    return {
        "clue": clue.word, "count": clue.count, "intended": clue.intended,
...
def main():
    ...
    ap.add_argument("--configs", default="", help="comma-separated substrings; keep matching configs")
    ...
    for cfg in cfgs:
        ...
        for bi, board in enumerate(boards):
            ...
            if ev:
                rows.append(ev)
            print(f"  [{cfg['name']:22}] board {bi+1}/{args.n}: "
                  f"{ev['clue'] if ev else '—'} ({ev['count'] if ev else '-'})", flush=True)
        results[cfg["name"]] = agg(rows)
        samples[cfg["name"]] = [(r["clue"], r["count"], r["intended"], r["intended_hit"]) for r in rows[:6]]

    report(results, samples, args)
    with open(args.out, "w", encoding="utf-8") as f:
        json.dump({"args": vars(args), "results": results, "samples": samples}, f, ensure_ascii=False, indent=2)
    print(f"\nsaved → {args.out}")
...
def trim_report(A, boards, args):
    ...
    def score(clue_word, intended):                                    # cross-engine guesser eval
        ...
        return dict(count=count, one=count == 1, delivered=hit, recovery=hit / count,
                    safe=all(board.role[w] == "my" for w in picks), assassin=board.assassin in picks)

    for bi, board in enumerate(boards):
        clue = probe.encoder_spymaster(enc, board, words, emb, vocab_lemmas=lems,
                                       lam_f=0.15, vocab_freq=fz, m=args.gen_m)
        if clue is None:
            continue
        ...
        cands = probe.encoder_clue_candidates(enc, board, words, emb, vocab_lemmas=lems,
                                              vocab_freq=fz, lam_f=0.15, n=10, m=args.gen_m)
        analyzed = [(c["word"], probe.served_count(read_of(c["word"]), keep_rel=keep,
                                                   enc=enc, cohesion_floor=floor)) for c in cands]
        ...
        for v in VARIANTS:
            ...
            row[v] = f"{max(1, len(served[v]))}·{'/'.join(served[v])}" + (f"={pick_word}" if v == "coh+order" else "")
        if len(ex) < 8:
            ex.append(row)
        print(f"  board {bi+1}/{len(boards)}: {clue.word}  raw={len(served['raw'])} "
              f"keep={len(served['keep_rel'])} coh={len(served['cohesion'])} "
              f"prod={pick_word}·{len(served['coh+order'])}", flush=True)
    ...
    for v in VARIANTS:
        rows = acc[v]
        if not rows:
            print(f"{v:12}  (no results)"); continue
        print(f"{v:12} "
...
def report(results, samples, args):
    ...
    for name, r in results.items():
        if not r.get("n"):
            print(f"{name:24}  (no results)"); continue
