...
def main() -> int:
    ...
    blocked = {
        line.strip() for line in BLOCKLIST.read_text(encoding="utf-8").splitlines()
    ...
    for row in rows:
        if len(row) != 3:
            continue
        word, count, pos = row
        if (100 <= count <= 150000 and len(word) >= 3
                and pos in {"NOUN", "ADJ", "PROPN"} and word not in blocked):
            words.append([word, count, pos])
    document = {
        "source": "content_master_v2_30000.json",
        "filters": {"min_count": 100, "max_count": 150000,
                    "pos": ["NOUN", "ADJ", "PROPN"], "min_len": 3,
