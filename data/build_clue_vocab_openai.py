...
def request_batch(
    api_key: str, model: str, category: str, n: int, existing: list[str], shortlist: list[str]
) -> list[dict]:
...
"""
    body = {
        ...
        "messages": [
            {
                "role": "system",
                "content": "Return conservative machine-readable Hebrew lexical metadata.",
            },
...
def main() -> int:
    ...
    eligible = [
        word
        for word, meta in source.items()
        if meta["count"] >= 300
        and meta["pos"] in {"NOUN", "ADJ", "PROPN"}
        and word not in blocked
        and word not in deck
        and " " not in word
    ]
    ...
    for batch in range(args.batches):
        ...
        if not pool:
            print(
                f"batch {batch + 1}/{args.batches}: eligible pool exhausted, stopping", flush=True
            )
            break
        ...
        for attempt in range(args.retries + 1):
            try:
                rows = request_batch(
                    api_key, args.model, category, args.per_batch, list(entries), shortlist
                )
                accepted = 0
                for row in rows:
                    ...
                    if (
                        ...
                        or not all(
                            0.0 <= x <= 1.0 for x in (familiarity, ambiguity, translation_risk)
                        )
                    ...
                    entries[word] = {
                        ...
                        "translation_risk": round(translation_risk, 3),
                        "flags": sorted(flags),
                        "origin": origin,
                        "in_corpus": word in source,
                        "source": "openai_curated",
                        "model": args.model,
                    }
                    accepted += 1
                print(
                    f"batch {batch + 1}/{args.batches}: accepted {accepted} (total {len(entries)})",
                    flush=True,
                )
                break
            except (
