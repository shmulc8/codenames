...
PROVIDERS = {
    ...
    "openrouter": ("https://openrouter.ai/api/v1/chat/completions", "OPENROUTER_API_KEY", "tencent/hy3:free"),
}
SYSTEM = (
...
def external_ranking(api_url: str, api_key: str, model: str, board: probe.Board, clue: str,
                     disable_reasoning: bool = False) -> list[str]:
    ...
    try:
        items = json.loads(re.sub(r"^```(?:json)?\\s*|\\s*```$", "", text.strip())).get("ranking", [])
...
def live_clue(board: probe.Board, risk: str) -> dict:
    ...
    candidates = probe.encoder_clue_candidates(
        app.get_enc(app.GEO_ENC), board, vocabulary, embeddings,
        vocab_lemmas=lemmas, vocab_freq=frequencies, lam_f=0.14, n=10, **kwargs)
    options = [app._analyze_clue(board, candidate["word"], candidate["intended"],
                                 candidate["count"], candidate["score"], None,
                                 keep_rel=profile["keep"], max_count=profile["m"])
               for candidate in candidates]
    if risk == "bold":
        key = lambda option: (option["no_clue"], -option["count"], -option["safe"], option["score"])
    else:
        key = lambda option: (option["no_clue"], option["risky"], -option["safe"],
                              -option["count"], -option["score"])
...
def main() -> None:
    ...
    for number in range(args.boards):
        ...
        for word in ranking:
            ...
            safe_run += 1
        rows.append({
            "clue": option["word"], "claimed": count,
            "intended": option["intended"], "recovered": len(set(picks) & set(option["intended"])),
            "safe_run": safe_run,
        ...
        print(f"{number + 1:>2}/{args.boards}: {option['word']}·{count} "
              f"safe={safe_run} recovered={rows[-1]['recovered']}/{count} "
              f"complete={rows[-1]['complete_ranking']}", flush=True)
        time.sleep(0.1)

    summary = {
        "provider": args.provider, "model": model, "boards": args.boards, "seed": args.seed, "risk": args.risk,
        ...
        "target_recovery": round(mean(row["recovered"] / max(1, row["claimed"]) for row in rows), 3),
