...
def main():
    ...
    for i, board in enumerate(boards):
        ...
        for key in encoder_keys:
            ...
            print(f"  {key:14s} -> clue={c.word!r:12} intended={list(c.intended)} rec={rec:.2f} safe={safe}", flush=True)

    print("\n" + "="*60, flush=True)
    print("=== SUMMARY METRICS ===", flush=True)
    print("="*60, flush=True)
    headers = ("encoder", "recovery", "spearman_rho", "assassin_rank", "safe_rate")
    print(f"{headers[0]:14s}  {headers[1]:8s}  {headers[2]:12s}  {headers[3]:13s}  {headers[4]:9s}", flush=True)
    ...
    for key in encoder_keys:
        ...
        print(f"{key:14s}  {mean_rec:8.2%}  {mean_rho:+12.3f}  {mean_assassin:13.1f}/25  {mean_safe:9.1%}", flush=True)
