...
def result_to_dict(h: HintEntry, r: EvalResult) -> dict:
    return {
        ...
        "dangerous_links": [
            {"word": w.word, "category": w.category, "strength": w.strength}
            for w in r.dangerous_links
        ],
...
def run(
    ...
    out_path.write_text(
        json.dumps(
            {"model": model, "risk": risk, "boards": all_boards}, ensure_ascii=False, indent=2
        ),
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate the spymaster engine with the v1 LLM judge."
    )
