...
import numpy as np
from probe import (make_encoder, load_clue_vocab, sample_board, encoder_spymaster,
                   encoder_rank, llm_guess_ranking, spearman, recovery_at_k, HebrewLLM, LLM_FAST)

KEYS = ["fasttext", "neodictabert", "embeddinggemma", "qwen3-embed"]
vocab = load_clue_vocab(3000, min_len=3)[300:]
encs, emb = {}, {}
for k in KEYS:
    t = time.time(); encs[k] = make_encoder(k); emb[k] = encs[k].embed(vocab)
    print(f"loaded {k:14s} {time.time()-t:5.1f}s", flush=True)
llm = HebrewLLM(LLM_FAST); print("llm loaded", flush=True)
...
for i, b in enumerate(boards):
    for k in KEYS:
        ...
        agg[k].append((recovery_at_k(llm_order, c.intended, c.count),
                       spearman(enc_order, llm_order),
                       llm_order.index(b.assassin), c.word, tuple(c.intended)))
    print(f"board {i} done", flush=True)

print("\n=== Encoder -> LLM : which embedding's clues does the LLM best recover? ===", flush=True)
rows = []
for k in KEYS:
    rec = np.mean([x[0] for x in agg[k]]); rho = np.mean([x[1] for x in agg[k]]); ar = np.mean([x[2] for x in agg[k]])
    rows.append((k, rec, rho, ar))
    print(f"{k:14s} llm_recovery={rec:.2f}  spearman={rho:+.3f}  assassin_rank={ar:.1f}/25", flush=True)
    for x in agg[k][:2]:
